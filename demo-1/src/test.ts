...
import { Cargaison, client, Client, Produit1, Aerienne, Maritime, Routiere, Alimentaire, Chimique, Materiel, Fragile, Incassable } from "./Model/Cargaison.js";
  ...
  cargaisonsPage.forEach(cargaison => {
    ...
    if (cargaison.etatGlobal === 'fermé' || cargaison.etatAvancement === 'perdue'
     || cargaison.etatAvancement === 'arrivé' || cargaison.etatAvancement === 'en cours') {
      const plusButton = row.querySelector('.fas') as HTMLElement;
...
function fermerModalAjout(): void {
  const modal = document.getElementById('mymodal1') as HTMLDialogElement;
  if (modal) {
...
const formProduct = document.getElementById('addProductForm') as HTMLFormElement;


formProduct?.addEventListener('submit', function (event) {
  const addProduct = document.getElementById('addProduct');
  const productPoids: number = parseFloat((document.getElementById('productWeight') as HTMLInputElement).value);
  const productToxicity: number = parseFloat((document.getElementById('productToxicity') as HTMLInputElement).value);
  ...
  const emailDestin = (document.getElementById('emailDestin') as HTMLInputElement).value;
  ...
  const emetteur: Client = { FirstName: clientFirstName, LastName: clientLastName, Phone: clientPhone, Adress: clientAdress, Mail: clientMail }
  const destinataire: Client = { FirstName: nomDestin, LastName: prenomDestin, Phone: clientPhone, Adress: addressDestin, Mail: emailDestin }
  ...
  fetch('../api.php')
    ...
    .then(data => {
      ...
      const msg: string = "ce type de produit ne peut etre transporter par cette cargaison";
...
function showAlert1(message: string, icone: string) {
   Swal.fire({
    title: "change",
    text: message,
    icon: icone,
...
function afficherProduit(produit: Produit1[], page = 1, tbodyElement?: HTMLElement, idCargo?: string): void {
